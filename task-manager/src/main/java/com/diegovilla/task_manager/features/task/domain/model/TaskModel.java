 ...
 *
 * <p>Encapsulates core task attributes including title, description, lifecycle status, owner user
 * identifier, and audit timestamps. Enforces domain invariants and valid status state transitions.
 *
 * @since 1.0.0
 */
@Getter
public class TaskModel {
    ...
    private TaskModel(
        ...
        this.description =
                Objects.requireNonNull(description, "Task description cannot be null").trim();
     ...
     *
     * <p>Validates that both {@code title} and {@code description} are present and within the
    ...
    /**
     * Reconstructs a {@link TaskModel} from persisted data without re-validating business rules.
     *
     ...
     *
     * @throws DomainException if the task has not been started or is already completed.
     */
     ...
     *
     * <p>Only non-{@code null}, non-blank values are applied. Completed tasks cannot be modified.
     *
    ...
    public void updateInformation(String title, String description) {
        ...
        this.description =
                ValidateDataUtils.updateIfPresent(
                        description, this.description, 3, 400, "Description");
        this.updatedAt = Instant.now();
    }

    /**
     * Returns a normalized (trimmed and lowered) version of the title suitable for uniqueness
     * checks.
     *
