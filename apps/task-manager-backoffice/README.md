...
- **End-to-End (E2E) Automation with Playwright**:
  - Full authentication and session lifecycle tests using Page Object Model (`e2e/pages/login.page.ts`).
  - Validates login success, error alert presentation on invalid credentials, and field validations against live backend instances.
...
├── playwright.config.ts               # Playwright E2E configuration with webServer
├── package.json                       # Dependencies, scripts and metadata
├── vercel.json                        # Vercel SPA routing rewrite rules
├── Dockerfile                         # Containerized Node 22 Alpine dev setup
├── e2e/                               # Playwright End-to-End test suite
│   ├── login.spec.ts                  # Admin login journeys & edge cases
│   └── pages/                         # Page Object Models (login.page.ts)
├── src/
...
```bash
# Start Backoffice dashboard
pnpm start:backoffice
# Or: nx start:dev task-manager-backoffice

# Run unit tests across backoffice
nx test task-manager-backoffice

# Build production bundle
nx build task-manager-backoffice
```

### Running inside `apps/task-manager-backoffice/`:
...
pnpm start                  # Default local environment (Port: 4200)
...
pnpm run test               # Run all tests once with ChromeHeadlessCI
pnpm run test:unit          # Run unit tests (excluding integration specs)
pnpm run test:watch         # Run unit tests in watch mode
pnpm run test:coverage      # Generate code coverage report (98%+ required)

# End-to-End Testing (Playwright)
pnpm run e2e                # Run Playwright E2E tests headless
pnpm run e2e:ui             # Interactive Playwright UI Mode
pnpm run e2e:debug          # Step-by-step debugger
pnpm run e2e:report         # Open HTML execution report
...
The Backoffice can run containerized:

```bash
# Build and run Docker container
docker build -t task-manager-backoffice .
docker run -p 4200:4200 task-manager-backoffice

# Or orchestrated via Docker Compose from root:
docker compose up -d --build angular-backoffice
```
...
The Backoffice is automatically verified and deployed to **Vercel** with strict Quality Gates:

1. **Static Checks & Quality Gate**:
   - `Fast-Fail Static Checks`: Prettier, ESLint, and strict TypeScript `tsc --noEmit`.
   - `Unit Tests`: Executes 164 unit tests with ChromeHeadlessCI (98%+ coverage required).
   - `Production Build`: Compiles Angular with production optimizations and budget checks.
   - `SonarCloud Scan`: Verifies maintainability, security, and coverage metrics.
2. **Ephemeral Full-Stack E2E**:
   - Deploys an isolated `global_postgres` container and compiles the Spring Boot API inside GitHub Actions.
   - Installs Chromium with OS dependencies via Playwright.
   - Runs **Playwright E2E** tests against the live API, uploading artifact reports on failure.
3. **Automated Deployments**:
   - **Pull Requests**: Deploys a **Preview URL** connected to the Dev backend and comments the link on the PR.
   - **`main` Branch**: Deploys directly to the **Production URL** on Vercel connected to the Production backend.
4. **SPA Routing**: `vercel.json` ensures all Angular deep links (`/dashboard/users`, `/dashboard/tasks`) are correctly rewritten to `/index.html`.
