...
1. **Object Equality**: By default, `signal()` uses strict equality (`===`). If you do `user.set({ name: 'Diego' })`, and then do `user.set({ name: 'Diego' })` again, Angular will consider them _different_ objects and trigger a re-render. If you want custom equality, you can pass `{ equal: (a, b) => a.id === b.id }` when creating the signal.
