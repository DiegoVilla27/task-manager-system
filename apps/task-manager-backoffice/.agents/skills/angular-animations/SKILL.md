...
export const fadeAnimation = animation([style({ opacity: "{{ startOpacity }}" }), animate("{{ time }} {{ timingFunction }}", style({ opacity: "{{ endOpacity }}" }))], {
  // Default parameters
  params: {
    startOpacity: 0,
    endOpacity: 1,
...
@Component({
  ...
  animations: [trigger("modalFade", [transition(":enter", [useAnimation(fadeAnimation, { params: { time: "500ms" } })]), transition(":leave", [useAnimation(fadeAnimation, { params: { startOpacity: 1, endOpacity: 0, time: "200ms" } })])])],
})
...
export const listAnimation = trigger("listAnimation", [
  ...
  transition("* <=> *", [
    ...
    query(":enter", [style({ opacity: 0, transform: "translateY(-15px)" })], { optional: true }), // <--- CRITICAL: Prevents crashes if list is empty

    // 2. Animate them in sequentially with a 50ms delay between each
    query(":enter", [stagger("50ms", [animate("300ms ease-out", style({ opacity: 1, transform: "translateY(0)" }))])], { optional: true }),
    ...
    query(
      ...
      [
        stagger("-50ms", [
          // Negative stagger removes them from bottom to top
...
_Usage in HTML: `<div [@listAnimation]="items.length">`_
...
export const shakeAnimation = trigger("shake", [transition("* => invalid", [animate("400ms", keyframes([style({ transform: "translateX(0)", offset: 0 }), style({ transform: "translateX(-10px)", offset: 0.2 }), style({ transform: "translateX(10px)", offset: 0.4 }), style({ transform: "translateX(-10px)", offset: 0.6 }), style({ transform: "translateX(10px)", offset: 0.8 }), style({ transform: "translateX(0)", offset: 1.0 })]))])]);
```
...
export const slideInAnimation = trigger("routeAnimations", [
  transition("* <=> *", [
    ...
    group([query(":leave", [animate("300ms ease-out", style({ transform: "translateX(-100%)", opacity: 0 }))], { optional: true }), query(":enter", [animate("300ms ease-out", style({ transform: "translateX(0)" }))], { optional: true })]),
  ]),
]);
```
