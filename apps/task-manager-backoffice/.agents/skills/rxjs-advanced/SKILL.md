...
_(Note: If you use `takeUntilDestroyed` outside the `constructor`, you MUST pass the `DestroyRef`: `takeUntilDestroyed(this.destroyRef)`)._
...
1. **CombineLatest vs ForkJoin**: Use `combineLatest` when you have multiple streams that emit _over time_ and you need the latest value of all of them (e.g., Filtering a table). Use `forkJoin` when you have multiple HTTP requests (that complete) and you want to wait for ALL of them to finish in parallel before proceeding.
