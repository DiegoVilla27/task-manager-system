...
_(Available variables: `$index`, `$first`, `$last`, `$even`, `$odd`, `$count`)_.
...
```html
@switch (paymentStatus()) { @case ('SUCCESS') {
<app-success-receipt />
} @case ('PENDING') {
<app-loading-spinner />
} @default {
...
Angular 17 introduced **Deferrable Views**. You can now lazy load _individual components_ on demand.
...
_(Note: Adding `minimum` prevents UI flickering by ensuring the state stays visible for at least that duration)._
...
For the absolute best User Experience, you want the JS to be already downloaded _before_ the user interacts with it.
