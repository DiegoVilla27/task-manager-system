...
providers: [{ provide: API_URL, useValue: "https://api.enterprise.com/v1" }];
...
providers: [{ provide: NewApiService, useExisting: LegacyApiService }];
```
...
If you need to fetch configuration from a server _before_ the application renders, you must use the initializer.
