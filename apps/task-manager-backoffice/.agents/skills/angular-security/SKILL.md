...
_(Note: If you use In-Memory storage, you must implement a Silent Refresh mechanism via an invisible iframe or refresh token endpoint to keep the user logged in across tab refreshes)._
...
_(Usage: `<button *appHasRole="['ADMIN', 'MANAGER']">Delete System</button>`)_
