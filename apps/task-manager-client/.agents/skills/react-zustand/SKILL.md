...
_Do NOT use Zustand for Server State (API responses); use TanStack Query instead._
...
If you need multiple properties from the store, calling the hook multiple times is fine. However, returning an object from the selector creates a _new reference_ every render, causing infinite re-renders unless you use `useShallow` (Zustand v4.5+).
...
Zustand lives _outside_ of React's lifecycle. You don't need a hook to read or write to it. This is invaluable for Axios interceptors, WebSockets, or standard utility functions where React hooks are forbidden.
