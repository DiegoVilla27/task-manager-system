...
The scope provides immediate context about _where_ the change occurred.
...
- **Imperative Mood**: Write as if giving a command. Use "add", "fix", "change" instead of "added", "fixes", "changing". (e.g., _If applied, this commit will **[description]**_).
...
- **Body**: Use the body to explain _why_ the change is being made and the contrast with previous behavior. Wrap lines at 72 characters. Separate from the description with a blank line.
...
- **Indicator `!`**: Append a `!` to the type/scope immediately before the `:` to draw immediate attention.
  - _Example_: `feat(api)!: remove v1 endpoints`
