...
_Why?_ You get a single source of truth, runtime values (Object.values(STATUS)), autocomplete, and safe refactoring.
...
2. **No `@ts-ignore`**: Never use `@ts-ignore`. If you must bypass the compiler, use `@ts-expect-error` and document _why_.
