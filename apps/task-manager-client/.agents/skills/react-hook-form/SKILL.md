...
- **`watch('fieldName')`**: Triggers a re-render of the _entire component_ where it is called whenever the field changes.
- **`useWatch({ name: 'fieldName' })`**: Triggers a re-render _only at the level of the component that calls the hook_. Use this to isolate re-renders in massive forms.
