 ...
 *
 * <p>All endpoints are mapped under {@code /tasks} and produce/consume JSON. Request validation is
 * enforced through Bean Validation annotations on the incoming DTOs.
 *
 * @since 1.0.0
 */
@Validated
...
@Tag(
        name = "Tasks",
        description =
                "Exposes operations for managing tasks, tracking status transitions, and handling task lifecycles within the Task Manager platform.")
public class TaskController {
    ...
    public ResponseEntity<TaskWithUserResponseDTO> create(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(taskDtoMapper.modelToWithUserResponseDTO(response));
    }
    ...
    public ResponseEntity<TaskWithUserResponseDTO> update(
            @PathVariable UUID id, @Valid @RequestBody TaskUpdateRequestDTO dto) {
        ...
        return ResponseEntity.ok().body(taskDtoMapper.modelToWithUserResponseDTO(response));
    }
