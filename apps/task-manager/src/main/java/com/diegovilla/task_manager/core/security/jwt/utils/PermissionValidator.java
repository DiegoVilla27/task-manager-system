 ...
 *
 * <p>Determines query filtering constraints based on whether the authenticated principal is an
 * administrator or standard user, and validates ownership before mutation operations.
 *
 * @since 1.0.0
 */
@Component
...
public class PermissionValidator {
     ...
     *
     * <p>If the authenticated user has an {@code ADMIN} role, the provided filter user ID is
     * respected (allowing admins to search all tasks or filter by a specific user). If the
     * authenticated user has a {@code USER} role, any incoming filter user ID is ignored and forced
     * to the authenticated user's ID.
     *
     * @param filterUserId the optional target user ID requested in query filters.
     * @return the effective target user ID to apply in database query specifications, or {@code
     *     null} if searching all tasks.
     */
    ...
    /**
     * Validates whether the currently authenticated user has permissions to access or mutate a
     * task.
     *
     * <p>Users with {@code ROLE_ADMIN} are granted unrestricted access. Users with {@code
     * ROLE_USER} are restricted to tasks that they own.
     *
     * @param taskUserId unique identifier of the user who owns the task.
     * @throws AccessDeniedException if a regular user attempts to access a task owned by another
     *     user.
     */
