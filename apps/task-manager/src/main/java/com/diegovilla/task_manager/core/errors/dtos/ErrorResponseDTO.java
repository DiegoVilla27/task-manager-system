 ...
 *
 * <p>This DTO provides a consistent error contract for both framework and business exceptions
 ...
 * @param errors collection of field-level validation errors. Empty when the error is not related to
 *     request validation.
 * @since 1.0.0
 */
...
public record ErrorResponseDTO(
        @Schema(
                ...
                Instant timestamp,
        @Schema(description = "HTTP status code integer value", example = "400") Integer status,
        @Schema(description = "HTTP status phrase / category", example = "Bad Request")
                String error,
        @Schema(
                        description = "Human-readable summary message describing the error",
                        example = "The request contains validation errors.")
                String message,
        @Schema(description = "Detailed list of field-level validation errors, if applicable")
