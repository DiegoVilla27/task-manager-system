 ...
 *
 * <p>Handles user queries with pagination and security scoping, self-profile lookups, secure user
 * registration with password hashing, profile updates, and cascade or restricted user deletion.
 *
 * @since 1.0.0
 */
@Service
...
public class UserService {
    ...
    public Page<UserWithTaskCount> getAll(
            UserPaginationCommand userPaginationCommand, UserFiltersCommand filters) {
        Pageable pageable =
                PageRequest.of(userPaginationCommand.page(), userPaginationCommand.limit());
        ...
        UserFiltersCommand effectiveFilters =
                new UserFiltersCommand(filters.search(), targetUserId);
    ...
    /**
     * Retrieves a single user and their task count by unique identifier, validating authorization
     * permissions.
     *
    ...
    public UserWithTaskCount create(UserCreateCommand userCreateCommand) {
        if (userRepositoryPort.existsByEmailIgnoreCase(
                StringUtils.normalize(userCreateCommand.email()))) {
