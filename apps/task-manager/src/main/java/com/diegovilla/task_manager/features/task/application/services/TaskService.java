 ...
 *
 * <p>Orchestrates CRUD operations and status-transition workflows ({@code start}, {@code complete})
 * by delegating persistence to {@link TaskRepositoryPort} and enforcing domain rules defined in
 * {@link TaskModel}.
 *
 * <p>Read-only operations execute within a read-only transaction; write operations use a standard
...
public class TaskService {
    ...
    public Page<TaskWithUser> getAll(
            TaskPaginationCommand taskPaginationCommand, TaskFiltersCommand taskFiltersCommand) {
        Pageable pageable =
                PageRequest.of(taskPaginationCommand.page(), taskPaginationCommand.limit());
        ...
        TaskFiltersCommand effectiveFilters =
                new TaskFiltersCommand(
                        targetUserId, taskFiltersCommand.search(), taskFiltersCommand.status());
     ...
     * @throws TaskAlreadyExistsException if a task with the same title already exists
     *     (case-insensitive).
     */
    @Transactional
    public TaskWithUser create(TaskCreateCommand taskCreateCommand) {
        ...
        if (taskRepositoryPort.existsByTitleIgnoreCase(
                StringUtils.normalize(taskCreateCommand.title()))) {
     ...
     *
     * <p>If the title is being changed, uniqueness is re-validated before applying the update.
     *
     ...
     * @throws ResourceNotFoundException if no task exists with the given id.
     * @throws TaskAlreadyExistsException if the new title collides with an existing task.
     */
    @Transactional
    public TaskWithUser update(UUID id, TaskUpdateCommand taskUpdateCommand) {
        ...
        if (newTitle != null
                ...
                && taskRepositoryPort.existsByTitleIgnoreCase(StringUtils.normalize(newTitle))) {
            throw new TaskAlreadyExistsException();
        }

        taskFound
                .task()
                .updateInformation(taskUpdateCommand.title(), taskUpdateCommand.description());
     ...
     * @throws ResourceNotFoundException if no task exists with the given id.
     * @throws DomainException if the task cannot be started from its current status.
     */
    @Transactional
     ...
     * @throws ResourceNotFoundException if no task exists with the given id.
     * @throws DomainException if the task cannot be completed from its current status.
     */
    @Transactional
